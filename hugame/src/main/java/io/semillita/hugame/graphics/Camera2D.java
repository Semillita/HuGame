...
public non-sealed class Camera2D extends Camera {
	...
	public Point screenToWorldCoords(int screenX, int screenY) {
		...
		int worldY = (int) (screenY * (viewportSize.height / (float) screenHeight) + position.y - viewportSize.height / 2);
	...
	public Vector2f getPosition() { return new Vector2f(position.x, position.y); }
	
	public void setPosition(Vector2f position) { this.position = new Vector2f(position.x, position.y); }
	
	@Override
	public void update() {
		super.projectionMatrix = new Matrix4f().identity().ortho(-viewportSize.width / 2f, viewportSize.width / 2f, -viewportSize.height / 2f, viewportSize.height / 2f, 0.0f, 100.0f);

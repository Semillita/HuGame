...
public class CustomTabbedPaneUI extends FlatTabbedPaneUI {
  ...
  public void paintHoverBorder(Graphics graphics) {
    ...
    if (hovered) {
      ...
      Consumer<Graphics> drawFunction =
          switch (getSectionFor(mouseX, mouseY)) {
  ...
  private void paintTopHalfHoverBorder(Graphics graphics) {
    graphics.drawRect(
        2,
        spacePartition.tabAreaHeight() + 3,
        tabbedPane.getWidth() - 4,
  ...
  private void paintRightHalfHoverBorder(Graphics graphics) {
    ...
    graphics.drawRect(
        borderWidth + 1,
        tabAreaHeight + 3,
        spacePartition.rightHalfWidth() - 2,
  ...
  private void paintLeftHalfHoverBorder(Graphics graphics) {
    ...
    graphics.drawRect(
        2,
        tabAreaHeight + 3,
        spacePartition.leftHalfWidth() - 2,
        tabbedPane.getHeight() - tabAreaHeight - 5);
  }

  @Override
  protected void paintCardTabBorder(
  ...
  protected void paintContentBorder(Graphics g, int tabPlacement, int selectedIndex) {
    if (tabPane.getTabCount() <= 0
        ...
        || !clientPropertyBoolean(
            tabPane, TABBED_PANE_SHOW_CONTENT_SEPARATOR, showContentSeparator)) return;
    ...
    boolean hasFullBorder =
        clientPropertyBoolean(tabPane, TABBED_PANE_HAS_FULL_BORDER, this.hasFullBorder);
    int sh =
        scale(
            contentSeparatorHeight
                * 100); // multiply by 100 because rotateInsets() does not use floats
    Insets ci = new Insets(0, 0, 0, 0);
    rotateInsets(
        hasFullBorder ? new Insets(sh, sh, sh, sh) : new Insets(sh, 0, 0, 0), ci, tabPlacement);
    ...
    path.append(
        new Rectangle2D.Float(
            x + (ci.left / 100f),
            y + (ci.top / 100f),
            w - (ci.left / 100f) - (ci.right / 100f),
            h - (ci.top / 100f) - (ci.bottom / 100f)),
        false);
    ...
    if (getTabType() == TAB_TYPE_CARD) {
      ...
      Rectangle2D.Float innerTabRect =
          new Rectangle2D.Float(
              tabRect.x + csh,
              tabRect.y + csh,
              tabRect.width - (csh * 2),
              tabRect.height - (csh * 2));
  ...
  protected void uninstallListeners() {
    ...
    Arrays.stream(tabbedPane.getMouseListeners())
        .filter(AbstractWidgetMouseListener.class::isInstance)
        .forEach(tabbedPane::removeMouseListener);

    Arrays.stream(tabbedPane.getMouseMotionListeners())
        .filter(AbstractWidgetMouseListener.class::isInstance)
  ...
  public class CustomMoreTabsButton extends FlatMoreTabsButton {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      for (int i = 0; i < rects.length; i++) {
        if (!viewRect.contains(rects[i])) {
          if (lastIndex >= 0 && lastIndex + 1 != i) lastIndex = i;
    ...
    protected MenuItem createTabMenuItem(int tabIndex) {
      ...
      if (StringUtils.isEmpty(title)) {
        ...
        if (tabComp != null) title = findTabTitle(tabComp);
        if (StringUtils.isEmpty(title))
          title =
              tabPane
                  .getAccessibleContext()
                  .getAccessibleChild(tabIndex)
                  .getAccessibleContext()
                  .getAccessibleName();
        if (StringUtils.isEmpty(title) && tabComp instanceof Accessible)
          title = findTabTitleInAccessible((Accessible) tabComp);
        if (StringUtils.isEmpty(title)) title = (tabIndex + 1) + ". Tab";
      }
    ...
    private String findTabTitle(Component c) {
      String title = null;
      if (c instanceof JLabel) title = ((JLabel) c).getText();
      else if (c instanceof JTextComponent) title = ((JTextComponent) c).getText();
  ...
  public Section getSectionFor(int x, int y) {
    ...
    if (partition.orientation() == Orientation.HORIZONTAL) {
      ...
      } else if (x >= partition.rightThirdX()
          && x < partition.rightThirdX() + partition.rightThirdWidth()) {
    ...
    } else if (y >= partition.bottomThirdY()
        && y < partition.bottomThirdY() + partition.bottomThirdHeight()) {

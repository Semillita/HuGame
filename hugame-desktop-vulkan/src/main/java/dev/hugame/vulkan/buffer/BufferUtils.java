...
public class BufferUtils {
	private static final int STAGING_BUFFER_USAGE_FLAGS = VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
	private static final int STAGING_BUFFER_PROPERTIES_FLAGS = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT
	...
	public static VulkanPersistentlyMappedBuffer createPersistentlyMappedBuffer(
		...
		try (var memoryStack = stackPush()) {
			...
			createBuffer(graphics, bufferSize, usageFlags, propertiesFlags, bufferHandleBuffer, bufferMemoryHandleBuffer);
			...
			return new VulkanPersistentlyMappedBuffer(bufferHandle, mappedMemory);
		}
	}
	...
	public static VulkanBuffer createDynamicBuffer(VulkanGraphics graphics, int bufferSize, int usageFlags, int propertiesFlags) {
		try (var memoryStack = stackPush()) {
	...
	public static void fillWithStagingBuffer(
		...
		var stagingBuffer = createStagingBuffer(
				graphics,
				bufferSize,
				stagingBufferData -> stagingBufferData.put(data));
	...
	public static void destroyBuffer(VulkanGraphics graphics, VulkanBuffer buffer) {
		...
		if (buffer instanceof VulkanPersistentlyMappedBuffer vpmBuffer) {
			MemoryUtil.memFree(vpmBuffer.getMappedMemory());
		}
	}

	private static void createBuffer(VulkanGraphics graphics,
									 int bufferSize,
									 int usageFlags,
									 int propertyFlags,
		...
		try (var memoryStack = stackPush()) {
			var bufferCreateInto = VkBufferCreateInfo.calloc(memoryStack)
			...
			var memoryAllocateInfo = VkMemoryAllocateInfo.calloc(memoryStack)
					...
					.memoryTypeIndex(findMemoryType(device, memoryRequirements.memoryTypeBits(),
							propertyFlags));
			...
			if (vkAllocateMemory(
					...
					memoryAllocateInfo,
					null,
					bufferMemoryHandleBuffer) != VK_SUCCESS) {
	...
	private static void copyBuffer(VulkanGraphics graphics, long sourceBufferHandle, long destinationBufferHandle, long size) {
		try (var memoryStack = stackPush()) {
			var temporaryCommandBuffer = VulkanCommandBuffer.create(graphics);
			var copyBufferCommand = new CopyBufferCommand(sourceBufferHandle, destinationBufferHandle, size);

			temporaryCommandBuffer.record(graphics, copyBufferCommand);
			graphics.getDevice().submit(temporaryCommandBuffer, memoryStack);

...
public class VulkanInstance {
	public static VulkanInstance create(boolean validationLayersEnabled) {
		try (var memoryStack = stackPush()) {
			...
			var instanceCreateInfo = VkInstanceCreateInfo.calloc(memoryStack)
					.sType(VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO)
					.pApplicationInfo(applicationInfo)
					.ppEnabledExtensionNames(getRequiredExtensionsBuffer(memoryStack, validationLayersEnabled));
	...
	private static PointerBuffer getRequiredExtensionsBuffer(MemoryStack memoryStack, boolean enableValidationLayers) {
		var glfwExtensionsBuffer = glfwGetRequiredInstanceExtensions();

		if (enableValidationLayers) {
			return memoryStack.mallocPointer(glfwExtensionsBuffer.capacity() + 1)

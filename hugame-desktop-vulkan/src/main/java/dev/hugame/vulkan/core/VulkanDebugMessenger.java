...
public class VulkanDebugMessenger {
	public static VulkanDebugMessenger create(VulkanInstance instance) {
		try (var memoryStack = stackPush()) {
			...
			if (vkGetInstanceProcAddr(instance.get(), "vkCreateDebugUtilsMessengerEXT") == VK_NULL_HANDLE) {
				throw new RuntimeException("[HuGame] Failed to find Vulkan function vkCreateDebugUtilsMessengerEXT");
			}
			...
			if (vkCreateDebugUtilsMessengerEXT(
					...
					null,
					debugMessengerHandleBuffer) != VK_SUCCESS) {
	...
	public static VkDebugUtilsMessengerCreateInfoEXT makeDebugMessengerCreateInfo(MemoryStack memoryStack) {
	...
	private static int messageCallback(int messageSeverity, int messageType, long callbackDataHandle, long pUserData) {
		var severity = switch (messageSeverity) {
		...
		System.err.printf("[HuGame] Vulkan validation (%s): %s%n", severity, callbackData.pMessageString());

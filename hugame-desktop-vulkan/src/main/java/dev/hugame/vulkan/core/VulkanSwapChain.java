...
public class VulkanSwapChain {
	...
	public static VulkanSwapChain create(VulkanGraphics graphics, long windowHandle) {
		...
		try (var memoryStack = stackPush()) {
			var swapChainCreateInfo = VkSwapchainCreateInfoKHR.calloc(memoryStack)
			...
			if (vkCreateSwapchainKHR(device.getLogical(), swapChainCreateInfo, null, swapChainHandleBuffer)
					!= VK_SUCCESS) {
			...
			vkGetSwapchainImagesKHR(logicalDevice, swapChainHandle, swapChainImageCountBuffer, swapChainImageHandleBuffer);

			var swapChainImageHandles = IntStream.range(0, swapChainImageCount)
			...
			for (int i = 0; i < swapChainImageCount; i++) {
				...
				if (vkCreateImageView(logicalDevice, imageViewCreateInfo, null, imageViewHandleBuffer) != VK_SUCCESS) {
	...
	private static VkSurfaceFormatKHR chooseSwapSurfaceFormat(List<VkSurfaceFormatKHR> availableFormats) {
		if (availableFormats.isEmpty()) {
			throw new RuntimeException("[HuGame] Failed to find surface format");
		}

		return availableFormats.stream()
				.filter(format -> format.format() == VK_FORMAT_B8G8R8A8_SRGB && format.colorSpace() == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR)
				.findFirst()
	 ...
	 * */
	private static VkExtent2D chooseSwapExtent(VkSurfaceCapabilitiesKHR capabilities, long windowHandle) {
	...
	// TODO: Make the swap chain hold a list of framebuffers, and the framebuffers hold their own image handles
	...
	private VulkanSwapChain(long handle,

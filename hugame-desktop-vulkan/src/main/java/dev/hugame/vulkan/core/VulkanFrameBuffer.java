...
public class VulkanFrameBuffer {
	...
	public static List<VulkanFrameBuffer> createAll(VulkanGraphics graphics) {
		...
		var renderPass = graphics.getPipeline().getRenderPass();
		...
		try (var memoryStack = stackPush()) {
			...
			for (long imageViewHandle : swapChainImageViewHandles) {
				var frameBufferCreateInfo = VkFramebufferCreateInfo.calloc(memoryStack)
				...
				if (vkCreateFramebuffer(
						...
						null,
						frameBufferHandleBuffer) != VK_SUCCESS) {

...
public class VulkanRenderPass {
	public static VulkanRenderPass create(VulkanGraphics graphics) {
		...
		try (var memoryStack = stackPush()) {
			...
			// Color attachment
			attachmentDescriptionBuffer.get(0)
					.format(graphics.getSwapChain().getImageFormat().format())
					.samples(VK_SAMPLE_COUNT_1_BIT)
					.loadOp(VK_ATTACHMENT_LOAD_OP_CLEAR) // TODO: Later change this to not clear every pass.
			...
			colorAttachmentReferenceBuffer.get(0)
					.attachment(0)
					.layout(VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);

			// Depth attachment
			attachmentDescriptionBuffer.get(1)
			...
			var depthAttachmentReference = VkAttachmentReference.calloc(memoryStack)
					.attachment(1)
					.layout(VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);

			var subpassDescriptionBuffer = VkSubpassDescription.calloc(1, memoryStack);
			subpassDescriptionBuffer.get(0)
			...
			var renderPassCreateInfo = VkRenderPassCreateInfo.calloc(memoryStack)
			...
			if (vkCreateRenderPass(
					...
					null,
					renderPassHandleBuffer) != VK_SUCCESS) {

...
public class VulkanPipeline {
	...
	public static VulkanPipeline create(VulkanGraphics graphics) {
		var logicalDevice = graphics.getDevice().getLogical();

		try (var memoryStack = stackPush()) {
			var vertexShaderSource = Files.read("/shaders/vulkan_demo_vertex_shader.glsl").orElseThrow();
			var fragmentShaderSource = Files.read("/shaders/vulkan_demo_fragment_shader.glsl").orElseThrow();
			...
			var fragmentShaderModuleHandle = ShaderFactory.createShaderModule(graphics, fragmentSpirVSource);

			var shaderStageBuffer = VkPipelineShaderStageCreateInfo.calloc(2, memoryStack);
			shaderStageBuffer.get(0)
					.sType(VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO)
					.stage(VK_SHADER_STAGE_VERTEX_BIT)
					.module(vertexShaderModuleHandle)
					.pName(memoryStack.UTF8(SHADER_ENTRYPOINT_METHOD_NAME));

			shaderStageBuffer.get(1)
					.sType(VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO)
					.stage(VK_SHADER_STAGE_FRAGMENT_BIT)
					.module(fragmentShaderModuleHandle)
					.pName(memoryStack.UTF8(SHADER_ENTRYPOINT_METHOD_NAME));

			var dynamicStateCreateInfo = VkPipelineDynamicStateCreateInfo.calloc(memoryStack)
					.sType(VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO)
					.pDynamicStates(memoryStack.ints(VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR));

			var vertexBindingDescriptions = VertexInput.getBindingDescriptions(memoryStack);
			var vertexAttributeDescriptions = VertexInput.getAttributeDescriptions(memoryStack);

			var vertexInputStateCreateInfo = VkPipelineVertexInputStateCreateInfo.calloc(memoryStack)
					.sType(VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO)
					.pVertexBindingDescriptions(vertexBindingDescriptions)
					.pVertexAttributeDescriptions(vertexAttributeDescriptions);

			var inputAssemblyStateCreateInfo = VkPipelineInputAssemblyStateCreateInfo.calloc(memoryStack)
					.sType(VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO)
					.topology(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST)
					.primitiveRestartEnable(false);

			var viewportStateCreateInfo = VkPipelineViewportStateCreateInfo.calloc(memoryStack)
					.sType(VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO)
					.viewportCount(1)
					.scissorCount(1);

			var rasterizationStateCreateInfo = VkPipelineRasterizationStateCreateInfo.calloc(memoryStack)
					...
					.frontFace(VK_FRONT_FACE_CLOCKWISE) // TODO: Investigate whether this is right
					.depthBiasEnable(false)
					.depthBiasConstantFactor(0)
					.depthBiasClamp(0)
					.depthBiasSlopeFactor(0);

			var multisampleStateCreateInfo = VkPipelineMultisampleStateCreateInfo.calloc(memoryStack)
			...
			var colorBlendAttachmentStateBuffer = VkPipelineColorBlendAttachmentState.calloc(1, memoryStack);
			colorBlendAttachmentStateBuffer.get(0)
			...
			var colorBlendStateCreateInfo = VkPipelineColorBlendStateCreateInfo.calloc(memoryStack)
					.sType(VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO)
					.logicOpEnable(false)
					.logicOp(VK_LOGIC_OP_COPY)
					.pAttachments(colorBlendAttachmentStateBuffer);

			var pipelineLayoutCreateInfo = VkPipelineLayoutCreateInfo.calloc(memoryStack)
					.sType(VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO)
					.pSetLayouts(memoryStack.longs(graphics.getDescriptorSetLayout().getHandle()))
			...
			if (vkCreatePipelineLayout(
					...
					null,
					pipelineLayoutHandleBuffer) != VK_SUCCESS) {
			...
			var depthStencilStateCreateInfo = VkPipelineDepthStencilStateCreateInfo.calloc(memoryStack)
			...
			pipelineCreateInfoBuffer.get(0)
			...
			if (vkCreateGraphicsPipelines(
					...
					null,
					pipelineHandleBuffer) != VK_SUCCESS) {

...
public class VulkanFence {
	public static VulkanFence create(VulkanGraphics graphics) {
		try (var memoryStack = stackPush()) {
			var fenceCreateInfo = VkFenceCreateInfo.calloc(memoryStack)
					.sType(VK_STRUCTURE_TYPE_FENCE_CREATE_INFO)
					.flags(VK_FENCE_CREATE_SIGNALED_BIT);

			var fenceHandleBuffer = memoryStack.callocLong(1);

			if (vkCreateFence(graphics.getDevice().getLogical(), fenceCreateInfo, null, fenceHandleBuffer) != VK_SUCCESS) {
